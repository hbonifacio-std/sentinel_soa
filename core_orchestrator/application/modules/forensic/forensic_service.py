...
class ForensicService(ForensicServicePort):
    ...
    async def get_analysis_history(self, query: ForensicHistoryQueryDTO) -> ForensicHistoryResponseDTO:
        ...
        return ForensicHistoryResponseDTO(**history)

    async def get_analysis_by_id(self, analysis_id: str, client_id: str) -> ForensicAnalysisRecordDTO | None:
        return await self.forensic_repository.get_analysis_by_id(analysis_id, client_id)

    async def _build_query_plan(self, request: ForensicAnalyzeRequestDTO, provider_override: dict | None = None) -> dict[str, Any]:
        try:
    ...
    async def _build_intelligence_report(
        self,
        forensic_analyzer_request: ForensicAnalyzeRequestDTO,
        total_matches: int,
    ...
    def _build_markdown_report(self, request: ForensicAnalyzeRequestDTO, total_matches: int, rows: list[dict]) -> str:
        lines = [
            "# Reporte Forense",
            "",
