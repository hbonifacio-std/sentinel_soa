...
from core_orchestrator.domain.exceptions.auth_exceptions import InvalidApiKeyError, ClientIdMismatchError, \
    InactiveTenantError
from core_orchestrator.domain.ports.auth.tenant_repository_port import TenantRepositoryPort
from core_orchestrator.infrastructure.dto.tenant.tenant_dto import TenantCreateRequestDTO, TenantResponseDTO, \
...
class TenantService:
    ...
    async def create_tenant(self, tenant_create: TenantCreateRequestDTO) -> TenantCreatedResponseDTO:
        """
    ...
    async def authorize_api_key(self, client_id: str, api_key:str) -> Optional[Tenant]:
        """
        Authorize an API key against a given client ID.

        This method validates if the provided API key belongs to the specified client ID. It performs
        several checks, including verifying the hashed API key, ensuring the client ID matches the
        key's owner, and confirming that the tenant associated with the key is active. If any of
        these conditions are not met, appropriate exceptions will be raised.

        Parameters:
            client_id: str
                The client ID to validate against the API key.
            api_key: str
                The raw API key to be authorized.

        Returns:
            Optional[Tenant]:
                The tenant entity associated with the provided API key, or None if not found.

        Raises:
            InvalidApiKeyError:
                Raised when the hash of the provided API key does not match any record.
            ClientIdMismatchError:
                Raised when the provided client ID does not match the one associated with the API key.
            InactiveTenantError:
                Raised when the tenant linked to the provided key is marked as inactive.
        """
        provider_hash = hashlib.sha256(api_key.encode("utf-8")).hexdigest()
        tenant_api_key_hash = await self.tenant_repository.get_by_api_key_hash(provider_hash)

        if not tenant_api_key_hash:
            raise InvalidApiKeyError(f"Hash {provider_hash[:8]}... not found.")

        if tenant_api_key_hash.client_id != client_id:
            raise ClientIdMismatchError(
                f"Client ID '{client_id}' does not match key owner '{tenant_api_key_hash.client_id}'."
            )

        if not tenant_api_key_hash.is_active:
            raise InactiveTenantError(f"Tenant '{client_id}' is marked as inactive.")

        return tenant_api_key_hash
