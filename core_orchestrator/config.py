...
class OrchestratorSettings(BaseSettings):
    ...
    def validate_command_structure(cls, v: str) -> str:
        ...
        cleaned = v.strip()
        if not cleaned:
            raise ValueError("El comando de ejecución de herramientas MCP no puede estar vacío.")
        return cleaned
    ...
    def validate_provider(cls, v: str) -> str:
        ...
        valid_providers = ['gemini', 'ollama'] # Ampliar a medida que se añadan más
    ...
    def validate_gemini_key(cls, v, values):
        if values.data.get('llm_provider') == 'gemini' and not v:
            raise ValueError("GEMINI_API_KEY es requerida cuando LLM_PROVIDER es 'gemini'")
        return v
