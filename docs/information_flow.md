...
1.  **Ingesta de Telemetría**:
    -   Un sistema externo (como un SIEM, un agente de logs, o una aplicación) envía un evento de log en formato JSON al endpoint `/telemetry/` del **Core Orchestrator**.

2.  **Gestión de Ventanas**:
    -   El **Core Orchestrator** recibe el log y lo añade a una "ventana de telemetría". Esta ventana agrupa logs que pertenecen a una misma sesión o que han ocurrido en un corto período de tiempo.
